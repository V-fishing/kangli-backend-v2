...
SELECT ops.gen_uuid_v7(), (SELECT id FROM ops.sys_menu WHERE menu_code='system'),
       'system.notify.center', '通知中心', '菜单', '/system/notify-center', 'views/system/NotifyCenter.vue', '📨', 96, true
WHERE NOT EXISTS (SELECT 1 FROM ops.sys_menu WHERE menu_code='system.notify.center');

INSERT INTO ops.sys_role_menu (id, role_id, menu_id)
SELECT ops.gen_uuid_v7(), rr.id, m.id
FROM ops.sys_role rr, ops.sys_menu m
WHERE m.menu_code='system.notify.center'
